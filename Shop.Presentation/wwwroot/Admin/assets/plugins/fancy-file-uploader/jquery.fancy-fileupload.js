	...
	$.fn.FancyFileUpload = function(options) {
		...
		$(window).on('beforeunload.fancy_fileupload', function(e) {
			...
			if (active.length || queued.length)
			{
				...
				if (active.length)  return Translate('بارگذاری فایل هنوز در حال انجام است. با ترک صفحه بارگذاری لغو خواهد شد.\n\nآیا مطمئن هستید که می خواهید این صفحه را ترک کنید؟');
				if (queued.length)  return Translate('فایلی وجود دارد که به صف اضافه شده است اما بارگذاری شروع نشده است. با ترک صفحه صف پاک می شود و فایل بارگذاری نمی شود.\n\nآیا مطمئن هستید که می خواهید این صفحه را ترک کنید؟');
			}
		});
		...
		var AddFile = function(uploads, e, data) {
			...
			var StartUpload = function(e) {
				...
				// Set the status.
				inforow.find('.ff_fileupload_fileinfo').text(data.ff_info.displayfilesize + ' | ' + Translate('شروع آپلود ...'));

				// Display progress bar.
				inforow.find('.ff_fileupload_progress_background').removeClass('ff_fileupload_hidden');

				// Alter remove buttons.
				inforow.find('button.ff_fileupload_remove_file').attr('aria-label', Translate('لغو آپلود و حذف از لیست'));
			...
			var RemoveFile = function(e) {
				...
				{
					if (!confirm(Translate('این فایل در حال بارگذاری است.\n\nبارگذاری را متوقف کرده و فایل را از لیست حذف می کنید؟')))  return;
				...
				else
				{
					...
					{
						if (!confirm(Translate('این فایل منتظر شروع است.\n\nعملیات را لغو کرده و فایل را از لیست حذف می کنید؟')))  return;
			...
			if (haspreview)
			{
				inforow.find('.ff_fileupload_preview_image').addClass('ff_fileupload_preview_image_has_preview').attr('aria-label', Translate('پیش نمایش')).click(function(e) {
					e.preventDefault();
			...
			{
				inforow.find('.ff_fileupload_preview_image').prop('disabled', true).attr('aria-label', Translate('پیش نمایش موجود نیست')).click(function(e) {
					e.preventDefault();
				});
			}
			...
			if (settings.accept)
			{
				...
				if (!found)  data.ff_info.errors.push(Translate('پسوند فایل نامعتبر است.'));
			}

			if (settings.maxfilesize > -1 && data.files[0].size > settings.maxfilesize)  data.ff_info.errors.push(FormatStr(Translate('فایل خیلی بزرگ است.  حداکثر سایز مجاز فایل {0} است.'), GetDisplayFilesize(settings.maxfilesize, settings.adjustprecision, settings.displayunits)));
			...
			{
				inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('شروع آپلود')).click(StartUpload));
				inforow.find('.ff_fileupload_actions_mobile').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('شروع آپلود')).click(StartUpload));

				inforow.addClass('ff_fileupload_queued');
			}

			inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_remove_file').attr('type', 'button').attr('aria-label', Translate('حذف از لیست')).click(RemoveFile));
			inforow.find('.ff_fileupload_actions_mobile').append($('<button>').addClass('ff_fileupload_remove_file').attr('type', 'button').attr('aria-label', Translate('حذف از لیست')).click(RemoveFile));
		...
		var UploadProgress = function(e, data) {
			...
			data.ff_info.fileinfo.text(FormatStr(Translate('{0} از {1} | {2}%'), GetDisplayFilesize(data.loaded, settings.adjustprecision, settings.displayunits), data.ff_info.displayfilesize, progress.toFixed(0)));
		...
		var UploadFailed = function(e, data) {
			...
			{
				data.ff_info.fileinfo.text(FormatStr(Translate('{0} | خطای شبکه، سعی مجدد پس از چند لحظه ... ({1})'), data.ff_info.displayfilesize, data.errorThrown));
			...
			{
				// Set the error info.
				if (data.errorThrown === 'abort')  data.ff_info.errors.push(Translate('آپلود فایل لغو شد'));
				else if (data.errorThrown === 'failed_with_msg')  data.ff_info.errors.push(FormatStr(Translate('آپلود شکست خورد.  {0} ({1})'), EscapeHTML(data.result.error), EscapeHTML(data.result.errorcode)));
				else  data.ff_info.errors.push(Translate('آپلود شکست خورد.'));
				...
				// Alter remove buttons.
				data.ff_info.inforow.find('button.ff_fileupload_remove_file').attr('aria-label', Translate('حذف از لیست'));
			}
		};
		...
		var UploadDone = function(e, data) {
			...
			{
				if (typeof(data.result.error) !== 'string')  data.result.error = Translate('سرور نشان داد آپلود موفقیت آمیز نبودد. هیچ اطلاعات اضافی در دسترس نیست.');
			...
			{
				// Set the status.
				data.ff_info.inforow.find('.ff_fileupload_fileinfo').text(data.ff_info.displayfilesize + ' | ' + Translate('آپلود فایل انجام شد'));
		...
		var UploadChunkSend = function(e, data) {
			if (data.ff_info)
			{
				...
				if (data.ff_info.lastresult && !data.ff_info.lastresult.success)
				{
					...
					if (typeof(data.ff_info.lastresult.error) !== 'string')  data.ff_info.lastresult.error = Translate('سرور نشان داد آپلود موفقیت آمیز نبودد. هیچ اطلاعات اضافی در دسترس نیست.');
		...
		return this.each(function() {
			...
			var dropzone = $('<button>').addClass('ff_fileupload_dropzone').attr('type', 'button').attr('aria-label', Translate('برای آپلود، مرور، کشیدن و رها کردن یا جایگذاری فایل ها'));
			...
			{
				var audiobutton = $('<button>').addClass('ff_fileupload_dropzone_tool').addClass('ff_fileupload_recordaudio').attr('type', 'button').attr('aria-label', Translate('ضبط صدا با استفاده از میکروفون'));
						...
						navigator.mediaDevices.getUserMedia({ audio: true }).then(function(stream) {
							...
							audiorec.addEventListener('dataavailable', function(e) {
								...
								if (audiorec.state === 'inactive')
								{
									...
									blob.name = FormatStr(Translate('ضبط صدا - {0}.mp3'), blob.lastModifiedDate.toLocaleString());
							...
							alert(Translate('ضبط صدا امکان پذیر نیست. یا میکروفون پیدا نشد یا دسترسی از آن رد شد.'));
						});
					}
					else
					{
			...
			{
				var videobutton = $('<button>').addClass('ff_fileupload_dropzone_tool').addClass('ff_fileupload_recordvideo').attr('type', 'button').attr('aria-label', Translate('ضبط فیلم با استفاده از دوربین'));
					...
					if (!videorec)
					{
						var streamhandler = function(stream) {
							...
							videorec.addEventListener('dataavailable', function(e) {
								...
								if (videorec.state === 'inactive')
								{
									...
									blob.lastModifiedDate = new Date();
									blob.lastModified = Math.floor(blob.lastModifiedDate.getTime() / 1000);
									blob.name = FormatStr(Translate('ضبط فیلم - {0}.mp4'), blob.lastModifiedDate.toLocaleString());
						...
						navigator.mediaDevices.getUserMedia({ video: true, audio: true }).then(streamhandler).catch(function(e) {
							navigator.mediaDevices.getUserMedia({ video: true }).then(streamhandler).catch(function(e) {
								alert(Translate('ضبط ویدئو امکان پذیر نیست. یا یک دوربین پیدا نشد یا دسترسی به آن رد شد.'));
							});
						});
					}
					else
